...
import { ArrowRight, BotMessageSquare, Check, ChevronRight, Clock3, MapPin, Send, Sparkles, X } from "lucide-react";
...
export function AiAdvisor({ onBook, navigationItem = false }: { onBook: (store: MarketplaceStore, offer?: BookingOffer) => void; navigationItem?: boolean }) {
  ...
  const [messages, setMessages] = useState<AdvisorMessage[]>([
    { id: "welcome", role: "advisor", text: "你好，我是美小天。最近有什么想改善的状态？" },
  ]);
  ...
  return <>
    <button className={navigationItem ? `ai-advisor-tab ${open ? "active" : ""}` : "ai-advisor-trigger"} onClick={() => setOpen(true)} aria-label="打开美小天" aria-expanded={open} title="美小天"><i>{navigationItem ? <BrandMark size="small" /> : <BotMessageSquare size={16} />}</i>{navigationItem && <span>小天</span>}</button>
    {open && <><button className="ai-advisor-scrim" onClick={() => setOpen(false)} aria-label="关闭美小天" /><aside className="ai-advisor-sheet">
      <header className="ai-advisor-head"><BrandMark /><div><strong>美小天</strong><small><i />美天美业 AI 护理顾问</small></div><button onClick={() => setOpen(false)} aria-label="关闭"><X size={19} /></button></header>
      ...
      <section className="ai-message-list">
        {messages.map((message) => <div className={`ai-message ${message.role}`} key={message.id}>{message.role === "advisor" && <BrandMark size="small" />}<div><p>{message.text}</p>{message.recommendations?.map((recommendation) => {
