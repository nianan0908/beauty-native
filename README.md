...
项目包含位于 `miniprogram/` 的全原生微信小程序：顾客端和门店助手均使用 WXML/WXSS/JavaScript，覆盖顾客预约与会员权益，以及员工、店长、老板移动工作台。在 `miniprogram/config.js` 中配置 Python API 地址后，可通过仓库根目录的 `project.config.json` 导入微信开发者工具。
